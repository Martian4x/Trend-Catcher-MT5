...
#property description "A dual moving average cross and RSI with timer, new bar check, money management, trailing stop and break even stop"
...
void OnTick()
{
	...
	if(newBar == true && timerOn == true)
	{
		...
		if(RSILevelSignalType == RSI_50){
			...
			// // If RSI Reverse direction in 50 level | Kill the Opened Trade
			// if(RSISignal=="Buy" && (RSI.Main(barShift+1) > 50.0 && RSI.Main(barShift) < 50.0 ) && PositionType() == POSITION_TYPE_BUY){ // Kill Any Buy Position
			// 	MTrade.PositionClose(_Symbol);
			// } else if(RSISignal=="Sell" && (RSI.Main(barShift+1) < 50.0 && RSI.Main(barShift) > 50.0 ) && PositionType() == POSITION_TYPE_SELL){ // Kill Any Sell Position
			// 	MTrade.PositionClose(_Symbol);
			// }
		}
		...
		// Open buy order
		if(glBuyPlaced == false && MASignal == "Buy" && RSISignal == "Buy")
		{
			// Close EA Sell Order
		...
		// }
		// Open sell order
		if(glSellPlaced == false && MASignal == "Sell" && RSISignal == "Sell")
		{
			// Close EA Buy Order
			if(PositionType() == POSITION_TYPE_BUY){
				MTrade.PositionClose(_Symbol);
